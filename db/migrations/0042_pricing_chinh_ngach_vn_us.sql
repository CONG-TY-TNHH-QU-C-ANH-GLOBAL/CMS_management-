...
--
-- All eight rows land as status = live, NOT draft. The CMS cannot change a rate
-- card's status: it is a read-only badge in the admin and savePricingTable()
-- never writes the column — the UPDATE sets data_json, schema_json and version
-- only. A draft insert would therefore be permanently invisible, because the
-- landing page renders a rate card only while it is live.
--
-- The review gate moves to this pull request instead: whoever approves it is
-- approving the figures for publication. Undo is `UPDATE pricing_tables SET
-- status = 'draft' WHERE slug LIKE 'chinhNgach%'`, which blanks the page's
