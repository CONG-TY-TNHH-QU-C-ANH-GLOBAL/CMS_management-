...
export const Route = createFileRoute("/api/v1/(public)/community/questions/$slug/same-issue")({
  server: {
    handlers: {
      ...
      // CORS-SIMPLE request (no body, `Accept` only) — the browser never preflights it, so the
      // boundary must reject the ACTUAL request. It runs before the `ip === "unknown"` check so
      // a disallowed origin never reaches the rate limiter or the reaction store.
