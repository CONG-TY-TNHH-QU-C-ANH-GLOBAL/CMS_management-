  ...
  server: {
    handlers: {
      ...
      POST: async ({ request, params }) => {
        ...
        const rl = await rateLimit("community-withdraw", ip, { max: 20, windowSeconds: 3600 });
        if (!rl.allowed) {
        ...
        const ok = await withdrawCommunityQuestion(params.slug, parsed.data.ownerToken);
        // Generic response either way — do NOT reveal whether the slug exists
        // or the token was merely wrong (no ownership enumeration).
        if (!ok) {
          return corsError(request, 404, "Không thể rút câu hỏi này.");
        }
        return corsJson(
          request,
