...
import { upsertCareersJobFn } from "@/features/careers/careers.actions";
...
export function NewCareersJobDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const ops = [upsert({ data: { slug, locale: "vi", title: titleVi.trim(), status: "open" } })];
      if (titleEn.trim()) ops.push(upsert({ data: { slug, locale: "en", title: titleEn.trim(), status: "open" } }));
      if (titleZh.trim()) ops.push(upsert({ data: { slug, locale: "zh", title: titleZh.trim(), status: "open" } }));
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 backdrop-blur-sm px-4" onClick={() => !pending && onOpenChange(false)}>
      <div className="w-full max-w-xl rounded-xl border border-border bg-background shadow-glow" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 pt-5 pb-3 border-b border-border">
          <h2 className="text-base font-semibold text-foreground">Tin tuyển dụng mới</h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            Bước 1: Nhập đường dẫn URL + tên vị trí (ít nhất bản Tiếng Việt). Bước 2: Bạn sẽ điền mô tả công việc, quyền lợi, yêu cầu... ở trang sau.
          </p>
        </div>
        <form onSubmit={onSubmit} className="px-5 py-4 space-y-3.5">
          <div>
            <label className="block text-xs font-medium text-foreground mb-1.5">Đường dẫn URL của vị trí</label>
            <input
              type="text"
              required
              value={slug}
          ...
          <div>
            <label className="block text-xs font-medium text-foreground mb-1.5">Tên vị trí (Tiếng Việt) <span className="text-red-600">*</span></label>
            <input
              type="text"
              required
              value={titleVi}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-foreground mb-1.5">Tên vị trí (English)</label>
            ...
            <div>
              <label className="block text-xs font-medium text-foreground mb-1.5">Tên vị trí (中文)</label>
